...
{
    public class EraseDeviceVM : NavigationVM
    {
        #region Nested types
                ...
                if (mBackupCommand == null)
                {
                    mBackupCommand = new RelayCommand(() =>
                    {
                        if (DriveManager.CheckAccess())
                        {
                            ...
                            var ownerWindow = Application.Current.Windows[Application.Current.Windows.Count - 1];
                            BackupToClickFreeWindow window = new BackupToClickFreeWindow(toFolder, SelectedDirList.Select(s => s.Path).ToList())
                            {
                                Owner = ownerWindow
                            };
                            window.ShowDialog();
        ...
        public EraseDeviceVM(INavigation navigation)
            : base(navigation)
        {
            Navigate(null);
        }
    ...
    public class CurrentDirectoiesHeaderTemplate1 : DataTemplateSelector
    {
        public override DataTemplate SelectTemplate(object item, DependencyObject container)
        {
            ...
            if (element != null && item is EraseDeviceVM.HeaderInfo hi)
            {
                try
                {
                    if (hi.IsStart)
